...
(function (global, factory) {
  ...
  function init(config, watch, options) {
    ...
    watch.$once("authSuccess", function (cb) {
      cb && cb();
    });
    ...
    keycloak$1.onAuthSuccess = function () {
      ...
      }; // Add callback when auth is successful


      typeof options.onAuthSuccess === "function" && watch.$emit("authSuccess", options.onAuthSuccess.bind(this, keycloak$1));
    };
    ...
    keycloak$1.init(options.init).error(function (err) {
    ...
    function updateWatchVariables() {
      ...
      watch.login = keycloak$1.login;
      watch.createLoginUrl = keycloak$1.createLoginUrl;
      watch.createLogoutUrl = keycloak$1.createLogoutUrl;
      watch.createRegisterUrl = keycloak$1.createRegisterUrl;
      watch.register = keycloak$1.register;

      if (isAuthenticated) {
        watch.accountManagement = keycloak$1.accountManagement;
        watch.createAccountUrl = keycloak$1.createAccountUrl;
        watch.hasRealmRole = keycloak$1.hasRealmRole;
        watch.hasResourceRole = keycloak$1.hasResourceRole;
        watch.loadUserProfile = keycloak$1.loadUserProfile;
        watch.loadUserInfo = keycloak$1.loadUserInfo;
        watch.token = keycloak$1.token;
        watch.subject = keycloak$1.subject;
        watch.idToken = keycloak$1.idToken;
        watch.idTokenParsed = keycloak$1.idTokenParsed;
        watch.realmAccess = keycloak$1.realmAccess;
        watch.resourceAccess = keycloak$1.resourceAccess;
        watch.refreshToken = keycloak$1.refreshToken;
        watch.refreshTokenParsed = keycloak$1.refreshTokenParsed;
        watch.timeSkew = keycloak$1.timeSkew;
        watch.responseMode = keycloak$1.responseMode;
        watch.responseType = keycloak$1.responseType;
  ...
  function assertOptions(options) {
    ...
    if (onInitError && typeof onInitError !== "function") {
      return {
        hasError: true,
        error: "'onInitError' option must be a function. Found: '".concat(onInitError, "'")
      };
    }
